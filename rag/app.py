...
def query():
    if request.method == "POST":
        ...
        env_data = request.json.get("env_data")
        if not user_query:
            return jsonify({"error": "No query provided"}), 400

        if env_data:
            r = pipeline.answer_question_with_env(user_query, env_data, store, provider)
            results = [r]
        elif mode == "fast":
            r = pipeline.answer_question_fast(user_query, store, provider)
            results = [r]
        else:
